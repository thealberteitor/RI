...
public class Searcher{
    ...
    public TopDocs IndexQueryParser(String campo , Analyzer analyzer) throws IOException {
        ...
        if(res.equals("si"))
            showFacets(tdc,query);
        return docs;
    }

    public void showFacets(TopDocs tdc, Query q1){
        Facets facetas = null;
        //inicializamos el DrillDownQuery con la consulta realizada
        DrillDownQuery ddq= new DrillDownQuery(fconfig,q1);
        BufferedReader in = new BufferedReader(new InputStreamReader(System.in , StandardCharsets.UTF_8));
        String [] vector_facetas =  new String[3*cuantos];
        int j=0;
        ...
        try{
            ...
            for( FacetResult fr : allDims){
                ...
                for(LabelAndValue lv : fr.labelValues){
                    if(cont < cuantos){
                        vector_facetas[j]=new String( fr.dim+ "::#->"+ lv.label + "");
            ...
            if(res.equals("si")){
                td2= filterSearchByFacets(ddq,td2, vector_facetas);
    ...
    public TopDocs filterSearchByFacets(DrillDownQuery ddq, TopDocs td2, String [] vector_facetas){
        ...
        try{
            ...
            for(int num=0 ; num < cuantos*3; num++){
                    if(vector_facetas[num]!= null)
                        System.out.println("\n[ "+ num +" ]"+ vector_facetas[num]);

                }


            System.out.println("Introduzca el/los filtro/s");
            entrada_teclado =  in.readLine();
            String[] filtros = entrada_teclado.split(" ");
            int[] facet_n= new int[filtros.length];


            for(int i=0;i<facet_n.length;i++){
                facet_n[i]= Integer.parseInt(filtros[i]);
                int iend = vector_facetas[facet_n[i]].indexOf(">"); //this finds the first occurrence of ">"
                int istart = vector_facetas[facet_n[i]].indexOf(":");
                String faceta = vector_facetas[facet_n[i]].substring(iend+1,vector_facetas[facet_n[i]].length());
                String categoria = vector_facetas[facet_n[i]].substring(0,istart);
            ...
            System.out.println("\n\n....(DrillDownQuery) Nueva busqueda  [ " + ddq.toString()+ " ]");
